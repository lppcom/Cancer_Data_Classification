...
M=t(exprs(ALL_Filter))
F=ALL_Filter$mol.biol


system.time({
    rf_fit <- foreach(ntree = rep(avg.tree, num.chunk), .combine =combine,.multicombine=TRUE, 
        .packages = c("randomForest")) %dopar% {
        randomForest(x=M,y=F, ntree = ntree)
    }
})

Fname=c("1636_g_at","1635_at","1467_at","39730_at","1249_at","35162_s_at","39837_s_at","40504_at","37363_at","37015_at")
syms = as.character(hgu95av2SYMBOL[Fname])
syms




cfKNN = table(rf_fit$predicted, ALL_Filter$mol.biol)
...
{
if (X>0.1) return(TRUE) else return(FALSE)
}


labels=c(rf_fit$importance[,1])

Filter(isGoodNumber,labels)
